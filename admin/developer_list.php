<?php
  ...
  if(isset($_POST['active'])){
    if(isset($_POST['chkbox'])){
      foreach ($_POST['chkbox'] as $key => $value) {
        $upd = "UPDATE `developer_master` SET `status`= '1' WHERE developerid = ".$value; 
  ...
  if(isset($_POST['inactive'])){
      if(isset($_POST['chkbox'])){
      foreach ($_POST['chkbox'] as $key => $value) {
        $upd = "UPDATE `developer_master` SET `status`= '2' WHERE developerid = ".$value;
        mysqli_query($conn,$upd);
      }
    }/*else{
      echo "<script>alert('Please select atleast one.');</script>"; 
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <a href="add_developer.php"  class="frt btn btn-success"> <i class="fa fa-plus"></i> Add developer</a>
          </div>
            ...
            <div class="card-body">
              <div class="table-responsive" style="overflow:hidden;">
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                  ...
                  <tbody id="checkboxes">
                    <?php
                      foreach ($res_developer as $key => $value){
                    ?>
                    <tr>
                      <td>  
                        <?php
                          if(!empty($value['developerid']))
                          {
                            //var_dump($value['developerid']);die();
                             $id=$value['developerid'] ;
                          }
                          //var_dump($value['developerid']);die();
                          ?>
                        <label>
                      ...
                      <td>
                      <a href="add_developer.php?id=<?php echo $value['developerid']?>" class="label label-info point"><i class="fa fa-edit" data-toggle="tooltip" title="Edit Record"></i></a>  
                      <!-- <a href="delete_room_type.php?id=<?php //echo $value['developerid']?>&action=roomtype_delete" class="label label-danger point">
                      <i class="fa fa-trash" data-toggle="tooltip" title="Delete Record"></i></a> -->

                      <a href="#"><i class="fa fa-trash delete" data-toggle="tooltip"  id='del_<?= $value['developerid'] ?>' data-id='<?= $value['developerid'] ?>'  title="Delete Record"></i></a>
