<?php
  ...
  if(isset($_POST['active'])){
    if(isset($_POST['chkbox'])){
      foreach ($_POST['chkbox'] as $key => $value) {
        $upd = "UPDATE `agent_master` SET `status`= '1' WHERE id = ".$value; 
  ...
  if(isset($_POST['inactive'])){
      if(isset($_POST['chkbox'])){
      foreach ($_POST['chkbox'] as $key => $value) {
        $upd = "UPDATE `agent_master` SET `status`= '2' WHERE id = ".$value;
        mysqli_query($conn,$upd);
      }
    }/*else{
      echo "<script>alert('Please select atleast one.');</script>"; 
  ...
  $sel_agent = "SELECT * FROM `agent_master` order By name asc";
  $res_agent = mysqli_query($conn,$sel_agent);

    $sel_develop = "SELECT * FROM `developer_master` order By name asc";
  $res_develop = mysqli_query($conn,$sel_develop);
?>
<!DOCTYPE html>
<html lang="en">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Agents Master</h1>
            <a href="add_agent.php"  class="frt btn btn-success"> <i class="fa fa-plus"></i> Add Agent</a>
          </div>
            ...
            <div class="card-body">
              <div class="table-responsive" style="overflow:hidden;">
                ...
                <table class="table table-bordered table-responsive" id="dataTable" width="100%" cellspacing="0">
                  ...
                  <tbody id="checkboxes">
                    <?php
                      foreach ($res_agent as $key => $value){
                    ?>
                    <tr>
                      <td>  
                        <?php
                      ...
                      <td><?php echo ucfirst($value['name']);?></td>
                       <td><?php echo ucfirst($value['role']);?></td>
                        ...
                        <td><?php echo ucfirst($value['phoneno']);?></td>
                         <td> <img src="img/<?php echo ucfirst($value['photo']);?>" style="width: 70px;height: 80px;"></td>
                      ...
                      <td>
                      <a href="add_agent.php?id=<?php echo $value['id']?>" class="label label-info point"><i class="fa fa-edit" data-toggle="tooltip" title="Edit Record"></i></a>  
                      <!-- <a href="delete_room_type.php?id=<?php //echo $value['cityid']?>&action=roomtype_delete" class="label label-danger point">
