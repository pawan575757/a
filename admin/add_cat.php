<?php
  ...
  if(isset($_POST['submit']) && isset($_GET['id'])){
    ...
    $qry_upd = "UPDATE `cat_master` SET `name` = '".$name."',`status` = '".$status."' WHERE `catid` = '".$_GET['id']."'"; 
    ...
    $sel_cat = "SELECT * FROM `cat_master` WHERE `name` = '".$name."' ";
    $res_cat = mysqli_query($conn,$sel_cat);
    $num_cat = mysqli_num_rows($res_cat);

    if($num_cat > 0){
      $msg = "cat already exist.";
    }else{
      $qry_ins = "INSERT INTO `cat_master` (`name`,`status`) VALUES ('".$name."','".$status."')"; 
  ...
  if(isset($_GET['id'])){
    $sel_cat = "SELECT `catid`, `name`, `status` FROM `cat_master` WHERE catid = '".$_GET['id']."' ";
    $res_cat = mysqli_query($conn,$sel_cat);
