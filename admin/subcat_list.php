<?php
  ...
  if(isset($_POST['active'])){
    if(isset($_POST['chkbox'])){
      foreach ($_POST['chkbox'] as $key => $value) {
        $upd = "UPDATE `subcat_master` SET `status`= '1' WHERE subcatid = ".$value; 
  ...
  if(isset($_POST['inactive'])){
      if(isset($_POST['chkbox'])){
      foreach ($_POST['chkbox'] as $key => $value) {
        $upd = "UPDATE `subcat_master` SET `status`= '2' WHERE subcatid = ".$value;
        mysqli_query($conn,$upd);
      }
    }/*else{
      echo "<script>alert('Please select atleast one.');</script>"; 
  ...
  $sel_subcat = "SELECT * FROM `subcat_master` order By subcatid desc";
  $res_subcat = mysqli_query($conn,$sel_subcat);

  $sel_cat = "SELECT * FROM `cat_master` order By catid desc ";
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Category Master</h1>
            <a href="add_subcat.php"  class="frt btn btn-success"> <i class="fa fa-plus"></i> Add Category</a>
          </div>
...
<!--                       <th>Primary Category</th>
 -->                      <th>Status</th>
                  ...
                  <tbody id="checkboxes">
                    <?php
                      foreach ($res_subcat as $key => $value){
                    ?>
                    <tr>
                      <td>  
                        <?php
                     ...
                     <!--  <td>  
                      ...
                      <td>
                      <a href="add_subcat.php?id=<?php echo $value['subcatid']?>" class="label label-info point"><i class="fa fa-edit" data-toggle="tooltip" title="Edit Record"></i></a>  
                      <!-- <a href="delete_room_type.php?id=<?php //echo $value['subcatid']?>&action=roomtype_delete" class="label label-danger point">
                      <i class="fa fa-trash" data-toggle="tooltip" title="Delete Record"></i></a> -->

                      <a href="#"><i class="fa fa-trash delete" data-toggle="tooltip"  id='del_<?= $value['subcatid'] ?>' data-id='<?= $value['subcatid'] ?>'  title="Delete Record"></i></a>
